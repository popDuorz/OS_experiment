...
class BarDescriptor(dict):
    """Bar descriptor
    To be used in leaf of a tree.py describing a hierarchy for ``ProgressTree``,
    e.g.,:
        tree.py = {"Job":
            {"Task1": BarDescriptor(...)},
            {"Task2":
...
class ProgressTree(object):
    ...
    For drawing a hierarchical progress view from a tree.py
    ...
    def _calculate_values(self, tree, bar_d):
        """Calculate values for drawing bars of non-leafs in ``tree.py``
        Recurses through ``tree.py``, replaces ``dict``s with
            ``(BarDescriptor, dict)`` so ``ProgressTree._draw`` can use
            the ``BarDescriptor``s to draw the tree.py
        """
        if all([
            isinstance(tree, dict),
            type(tree) != BarDescriptor
            ...
            for k in tree:
                ...
                # Reassign to tuple of (new descriptor, tree.py below)
    ...
    def _draw(self, tree, indent=0):
        """Recurse through ``tree.py`` and draw all nodes"""
        if all([
            isinstance(tree, dict),
            type(tree) != BarDescriptor
        ]):
